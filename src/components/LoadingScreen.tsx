...
import { Cpu, Users, ArrowRight, ShieldCheck, Zap } from "lucide-react";
...
export default function LoadingScreen() {
  ...
  useEffect(() => {
    // Show name first, then reveal the two destination choices
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#050811] px-4 py-8 overflow-y-auto transition-opacity duration-500 ${
        selectedAnimation ? "opacity-0 pointer-events-none scale-105" : "opacity-100"
      }`}
    >
      {/* Background ambient lighting */}
      <div className="absolute top-1/3 left-1/4 w-96 h-96 bg-eng-cyan/10 rounded-full blur-[140px] pointer-events-none"></div>
      <div className="absolute bottom-1/3 right-1/4 w-96 h-96 bg-amber-500/10 rounded-full blur-[140px] pointer-events-none"></div>

      <div className="max-w-4xl w-full mx-auto text-center space-y-8 relative z-10 my-auto">
        {/* Step 1: Centered Name */}
        <div className="space-y-3">
          <span className="font-mono text-xs tracking-widest text-slate-400 uppercase">
            ENGINEERING &amp; EXECUTIVE PORTFOLIOS
          </span>
          <h1 className="text-2xl sm:text-4xl md:text-5xl font-extrabold tracking-[0.18em] text-slate-100 uppercase font-sans">
            BABAWALE VICTOR AYOMIDE
          </h1>
          <div className="w-28 h-[2px] bg-gradient-to-r from-eng-cyan via-amber-400 to-eng-cyan mx-auto"></div>
        </div>

        {/* Step 2: Destination Prompt & Cards */}
        <div
          className={`space-y-6 transition-all duration-700 ${
            showCards ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6 pointer-events-none"
          }`}
        >
          <div className="flex items-center justify-center gap-2 font-mono text-xs text-slate-300">
            ...
            <span className="tracking-widest uppercase text-slate-300 font-semibold">
              SELECT YOUR PORTFOLIO DESTINATION //
            </span>
          </div>

          {/* The Two Themed Portal Cards */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
            {/* Card A: Embedded Systems Engineer */}
            <button
              onClick={() => handleSelect("embedded")}
              className="p-7 rounded-2xl bg-gradient-to-b from-[#0B132B]/90 via-[#07111F]/90 to-[#030712] border-2 border-eng-cyan/40 hover:border-eng-cyan hover:shadow-2xl hover:shadow-eng-cyan/20 transition-all group flex flex-col justify-between space-y-6 text-left relative overflow-hidden"
            >
            ...
            {/* Card B: Leadership & Community Governance */}
            <button
              onClick={() => handleSelect("leadership")}
              className="p-7 rounded-2xl bg-gradient-to-b from-[#161F33]/90 via-[#0F172A]/90 to-[#0A0E18] border-2 border-amber-500/40 hover:border-amber-400 hover:shadow-2xl hover:shadow-amber-500/20 transition-all group flex flex-col justify-between space-y-6 text-left relative overflow-hidden"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between font-mono text-xs">
